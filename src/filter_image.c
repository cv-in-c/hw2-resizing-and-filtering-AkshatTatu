    ...
    for (int y = 0; y < im.h; y++){
      ...
      for (int i = 0; i < diameter; i++) {
        for (int j = 0; j < diameter; j++) {
          ...
          double gi = exp(-((im.data[neighbor_x + neighbor_y*im.w + c*im.w*im.h]-im.data[x + y*im.w + c*im.w*im.h])*(im.data[neighbor_x + neighbor_y*im.w + c*im.w*im.h]-im.data[x + y*im.w + c*im.w*im.h]))/(sigmar*sigmar*2))/(sigmar*sigmar*TWOPI);
          double gs = exp(-((half-i)*(half-i)+(half-j)*(half-j))/(sigmad*sigmad*2))/(sigmad*sigmad*TWOPI);
          sum += gi*gs;
        }
      }
      for (int i = 0; i < diameter; i++) {
